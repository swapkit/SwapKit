...
description: Complete guide to integrating KeepKey hardware wallets with SwapKit SDK v4
---

KeepKey is a premium hardware wallet that provides secure cryptocurrency storage with a large OLED display for transaction verification. SwapKit SDK v4 supports KeepKey devices across multiple blockchain networks through the KeepKey SDK integration.

## Supported Chains

SwapKit supports KeepKey devices on these chains:
...
1. Download KeepKey Desktop from [keepkey.com](https://www.keepkey.com)
...
import { SwapKit, keepkeyWallet, Chain } from "@swapkit/sdk";
...
import { SwapKit, keepkeyWallet, Chain, NetworkDerivationPath } from "@swapkit/sdk";
...
import { SwapKit, keepkeyWallet, Chain, NetworkDerivationPath, type DerivationPathArray } from "@swapkit/sdk";
...
async function connectWithCustomPaths() {
  ...
  try {
    await swapKit.connectKeepkey(chains, derivationPaths);
...
```ts twoslash
import { SwapKit, keepkeyWallet, Chain, AssetValue } from "@swapkit/sdk";
...
await swapKit.connectKeepkey([Chain.Ethereum]);
...
await swapKit.connectKeepkey([Chain.Cosmos]);
...
await swapKit.connectKeepkey([Chain.THORChain]);
...
const swapKit = SwapKit({ wallets: { ...keepkeyWallet } });
...
async function setupEVMChains() {
  ...
  try {
    await swapKit.connectKeepkey(evmChains);

    const evmAddress = swapKit.getAddress(Chain.Ethereum);
...
async function setupUTXOChains() {
  ...
  try {
    await swapKit.connectKeepkey(utxoChains, derivationPaths);
...
async function verifyAddressOnDevice() {
  const swapKit = SwapKit(secureConfig);
  await swapKit.connectKeepkey([Chain.Bitcoin]);
...
class KeepKeyAccountManager {
  ...
  async connectAccount(accountIndex: number, chains: Chain[]) {
    ...
    await this.swapKit.connectKeepkey(chains, derivationPaths);

    const accountAddresses: Record<Chain, string> = {};
    for (const chain of chains) {
      accountAddresses[chain] = this.swapKit.getAddress(chain);
    }
...
export function KeepKeyWallet({ onConnect, onError }: KeepKeyWalletProps) {
  ...
  const connectKeepKey = async () => {
    ...
    try {
      await swapKit.connectKeepkey(selectedChains);
...
This comprehensive guide covers all aspects of KeepKey integration with SwapKit SDK v4, including setup, usage, troubleshooting, and advanced features. The KeepKey integration provides secure hardware wallet functionality with support for multiple blockchain networks through a user-friendly interface.
