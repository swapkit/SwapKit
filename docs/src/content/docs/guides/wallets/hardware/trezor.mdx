...
description: Complete guide to integrating Trezor hardware wallets with SwapKit SDK v4
---

Trezor hardware wallets provide secure cryptocurrency storage by keeping private keys offline on a dedicated device. SwapKit SDK v4 supports both Trezor Model One and Model T devices across multiple blockchain networks.

## Supported Chains

SwapKit supports Trezor devices on these chains:
...
Download and install [Trezor Suite](https://trezor.io/trezor-suite)
...
import { SwapKit, trezorWallet, Chain } from "@swapkit/sdk";
...
async function connectTrezorBitcoin() {
  try {
    ...
    await swapKit.connectTrezor([Chain.Bitcoin], derivationPath);
...
async function connectTrezorEthereum() {
  try {
    ...
    await swapKit.connectTrezor([Chain.Ethereum], derivationPath);
...
async function connectMultipleChains() {
  ...
  try {
    for (const { chain, derivationPath } of chainConfigs) {
      await swapKit.connectTrezor([chain], derivationPath);
...
async function connectWithCustomPath() {
  try {
    await swapKit.connectTrezor([Chain.Bitcoin], BITCOIN_PATHS.LEGACY);
    const legacyAddress = swapKit.getAddress(Chain.Bitcoin);
...
async function sendBitcoin() {
  ...
  try {
    const txHash = await swapKit.transfer({
      assetValue: amount,
      recipient,
      memo: "SwapKit Trezor Transaction",
    });
...
await swapKit.connectTrezor([Chain.Ethereum], NetworkDerivationPath.ETH);
...
await swapKit.connectTrezor([Chain.BitcoinCash], NetworkDerivationPath.BCH);
...
async function setupBitcoinAddressTypes() {
  ...
  for (const [type, path] of Object.entries(paths)) {
    try {
      await swapKit.connectTrezor([Chain.Bitcoin], path);
      const address = swapKit.getAddress(Chain.Bitcoin);
      results[type] = address;
      console.log(`${type} address:`, address);

      swapKit.disconnectChain(Chain.Bitcoin);
    } catch (error) {
...
async function setupEVMChains() {
  ...
  for (const chain of evmChains) {
    try {
      await swapKit.connectTrezor([chain], ethPath);
      const address = swapKit.getAddress(chain);
...
import { SwapKit, trezorWallet, Chain, NetworkDerivationPath } from "@swapkit/sdk";
...
async function transactionWithTimeout() {
  ...
  const transactionPromise = swapKit.transfer({
...
class CustomTrezorWallet {
  ...
  constructor() {
    this.swapKit = SwapKit({ wallets: { ...trezorWallet } });
  }
  ...
  async connectWithAccountIndex(chain: Chain, accountIndex: number) {
    ...
    await this.swapKit.connectTrezor([chain], basePath);
    return this.swapKit.getAddress(chain);
  }
  ...
  async connectMultipleAccounts(chain: Chain, accountCount: number) {
    ...
    for (let i = 0; i < accountCount; i++) {
      try {
        ...
        this.swapKit.disconnectChain(chain);
      } catch (error) {
...
export function TrezorWallet({ onConnect, onError }: TrezorWalletProps) {
  ...
  const connectTrezor = async () => {
    ...
    try {
      ...
      await swapKit.connectTrezor([selectedChain], derivationPath);
...
This comprehensive guide covers all aspects of Trezor integration with SwapKit SDK v4, including security best practices, troubleshooting, and advanced usage patterns.
