...
description: Complete guide to integrating OKX Wallet with SwapKit SDK v4 for multi-chain support
---

OKX Wallet is a comprehensive multi-chain browser extension wallet that supports a wide range of blockchain networks. SwapKit SDK v4 provides extensive support for OKX Wallet across all its supported chains, making it an excellent choice for multi-chain applications.

## Supported Chains

SwapKit supports OKX Wallet for these chains:
...
- **Chrome/Chromium**: [Chrome Web Store](https://chrome.google.com/webstore/detail/okx-wallet/mcohilncbfahbmgdjkbpemcciiolgcge)
- **Firefox**: [Firefox Add-ons](https://addons.mozilla.org/en-US/firefox/addon/okexwallet/)
...
```ts twoslash
import { SwapKit, okxWallet, Chain } from "@swapkit/sdk";
...
import { SwapKit, okxWallet, Chain, AssetValue } from "@swapkit/sdk";
...
This comprehensive guide covers all aspects of OKX Wallet integration with SwapKit SDK v4, including support for multiple chains, security best practices, and troubleshooting for common issues across different blockchain networks.
