...
### 1. Chains
SwapKit uses a unified chain identifier system:
...
// Get balance
const balance = await btcToolbox.getBalance(
  'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh'
);
```
...
try {
  const quote = { /* your quote object */ };
  await swapKit.swap(quote);
} catch (error) {
...
const address = '0x742d35Cc6634C0532925a3b844Bc9e7595f6E321';
...
```typescript
// @noErrorValidation
import type { Chain, AssetValue, SwapParams } from '@swapkit/helpers';

// Type-safe chain usage
function getChainName(chain: Chain): string {
  return chain; // TypeScript knows this is a valid chain
}

// Type-safe swap parameters
const swapParams = {
  sellAsset: 'ETH.ETH',
  buyAsset: 'BTC.BTC',
  sellAmount: '1',
  // TypeScript will help with autocompletion
} as const;
```
