...
async function signCosmosTransactionManually() {
  // Get Cosmos toolbox
  const cosmosToolbox = getCosmosToolbox(Chain.THORChain);

  // Create a wallet from a mnemonic phrase
  const phrase = "your twelve word mnemonic phrase here";
  const wallet = await cosmosToolbox.getSigner(phrase);
  const address = await cosmosToolbox.getAddressFromMnemonic(phrase);

  // Message to sign
  const message = "Sign this THORChain message";

  // Sign the message
  const signature = await cosmosToolbox.signMessage(message, wallet);

  // Verify the signature
  const isValid = await cosmosToolbox.verifySignature({ message, signature, address });
