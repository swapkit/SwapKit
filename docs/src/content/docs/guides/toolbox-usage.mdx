...
import { getToolboxByChain, getProvider } from "@swapkit/toolboxes/evm";
import { ethers } from "ethers";

// Get a toolbox for a specific EVM chain
const ethereumToolbox = getToolboxByChain(Chain.Ethereum)({
  // You can provide a custom provider or signer
  provider: getProvider(Chain.Ethereum),
});

// With a signer (for transactions)
const signer = new ethers.Wallet("your-private-key-here", getProvider(Chain.Ethereum));
const signingToolbox = getToolboxByChain(Chain.Ethereum)({
  provider: getProvider(Chain.Ethereum),
  signer,
});
```
...
const isValidAddress = evmValidateAddress("0x742d35Cc6634C0532925a3b844Bc454e4438f44e");
```
...
async function transferTokens() {
  ...
  const ethToolbox = getToolboxByChain(Chain.Ethereum)({
    provider,
    signer
  });
...
async function interactWithContract() {
  ...
  // Get balance
  const balance = await ethToolbox.call({
    contractAddress,
    abi: erc20Abi,
    funcName: "balanceOf",
  ...
  // Get decimals
  const decimals = await ethToolbox.call({
    contractAddress,
    abi: erc20Abi,
    funcName: "decimals",
  });
  ...
  console.log(`Balance: ${formattedBalance} ${tokenName}`);

  // Contract write operation (requires signer)
  const transferResult = await ethToolbox.call({
    contractAddress,
    abi: erc20Abi,
    funcName: "transfer",
    funcParams: [
...
async function estimateFees() {
  const provider = getProvider(Chain.Ethereum);
  const ethToolbox = getToolboxByChain(Chain.Ethereum)({ provider });
...
import { AssetValue, Chain } from "@swapkit/helpers";
import { getUtxoToolbox } from "@swapkit/toolboxes/utxo";
import { getToolboxByChain } from "@swapkit/toolboxes/evm";
import { ethers } from "ethers";
...
async function crossChainOperation() {
  ...
  const ethToolbox = getToolboxByChain(Chain.Ethereum)({ provider, signer });
