...
## Advanced AssetValue Operations

### Working with Base Decimals

For advanced use cases, you may need to work directly with base decimal values (Wei, Satoshi, etc.). AssetValue supports this through the `fromBaseDecimal` parameter:
...
const oneBtcInSatoshi = AssetValue.from({
  asset: "BTC.BTC",
  value: "100000000",
  fromBaseDecimal: true,
});
...
- Parsing blockchain transaction data
- Working with smart contract return values
- Interfacing with low-level toolbox operations
- Processing raw API responses

```typescript
// @noErrorValidation

const txData = await ethereumToolbox.getTransaction(txHash);
const gasUsed = AssetValue.from({
  asset: "ETH.ETH",
  value: txData.gasUsed.toString(),
  fromBaseDecimal: true,
});

const contractBalance = await contractInstance.balanceOf(address);
const balance = AssetValue.from({
  asset: "ETH.USDC-0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
  value: contractBalance.toString(),
  fromBaseDecimal: true,
});
```
...
```typescript
import { Chain, ChainId } from '@swapkit/sdk';
import { SwapKitApi } from '@swapkit/helpers/api';
...
- Check [Playgrounds](https://github.com/swapkit/SwapKit/tree/develop/playgrounds)
