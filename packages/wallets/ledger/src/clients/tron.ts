...
export class TronLedgerInterface implements TronSigner {
  ...
  getAddress = async (): Promise<string> => {
    ...
    if (!response) throw new SwapKitError("wallet_ledger_get_address_error");
  ...
  getAddressAndPubKey = async () => {
    ...
    if (!result) throw new SwapKitError("wallet_ledger_get_address_error");
  ...
  signTransaction = async (transaction: TronTransaction): Promise<TronSignedTransaction> => {
    ...
    // Ledger expects the raw_data_hex field from TronWeb transactions
    if (!transaction.raw_data_hex) {
      throw new SwapKitError("wallet_ledger_sign_error", {
        error: "Transaction missing raw_data_hex field",
      });
    }
    ...
    try {
      ...
      throw new SwapKitError("wallet_ledger_sign_error", { error });
    }
  };
}
